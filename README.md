# Hi 👋 I'm Maruti Auti  

🚀 Full Stack Web Developer | MCA Student | MERN Stack Enthusiast  

---

## 👨‍💻 About Me  
- Motivated **Software Developer** with a strong foundation in **Frontend & Full Stack Development**  
- Experienced in building **end-to-end web applications** using modern technologies  
- Passionate about **problem-solving, clean code, and continuous learning**  
- Enjoy working on **real-world projects** that solve practical problems  

---

## 🛠️ Technical Skills  

### 💻 Frontend Development  
- HTML5, CSS3, JavaScript  
- React.js (Hooks: `useState`, `useEffect`)  
- Responsive UI & user-friendly design  

### ⚙️ Backend Development  
- Node.js, Express.js  
- RESTful API development  
- JSON handling & API integration  

### 🗄️ Database  
- MongoDB (CRUD operations, schema design)  
- MySQL (Basic SQL)  

### 🧠 Programming Languages  
- JavaScript  
- Python (Basic)  

### 🔧 Tools & Technologies  
- Git & GitHub (commit, push, pull, branches)  
- VS Code  
- MongoDB Compass  

### 🤝 Other Skills  
- Logical thinking & debugging  
- Problem-solving mindset  
- Good communication & teamwork  

---

## 🚀 Projects  

- **FarmZy** – A digital platform that connects farmers directly with buyers to enable fair pricing and efficient agricultural trade.  

- **SmartSheti** – A smart farming web application that helps farmers make better decisions by providing soil information and crop suggestions through a user-friendly interface.  

- **Point of Sale (POS) System** – A web-based billing and inventory management system that helps retail shops manage products, generate bills, and track daily sales efficiently.  

- **Portfolio Website** – A personal portfolio website showcasing my skills, projects, and journey as a software developer.  

---

## 🎓 Education  
- **MCA (Master of Computer Application)**  
  G H Raisoni College of Engineering & Management, Pune | SPPU  
  *2024 – 2026 | CGPA: 7.91*  

- **BCA (Bachelor of Computer Application)**  
  B J College, Ale | SPPU  
  *2021 – 2024 | CGPA: 7.80*  

---

## 📜 Certifications  
- Web Development with React / Angular – Seven Mentor Pvt. Ltd.  
- Python Programming – Udemy  
- Java Programming – Udemy  

---

## 🔥 GitHub Streak
<img src="https://streak-stats.demolab.com?user=autimaruti2004&theme=tokyonight" />

---

## 📫 Connect with Me  
- 💼 LinkedIn: https://www.linkedin.com/in/maruti-auti-458b83234  
- 🌐 Portfolio: https://maruti-auti.vercel.app/  
- 📧 Email: autimaruti2004@gmail.com  
- 🧑‍💻 GitHub: https://github.com/autimaruti2004  

---

⭐ *Always eager to learn, build, and grow as a Full Stack Developer.*
